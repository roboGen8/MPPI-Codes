function [states_p,out_states_p]=predicted_vehicle_state(del_bi,del_ai,del_pi,del_ci,dt,states_p,out_states_p,j,Afull,Bfull,trim_val)
...
for k=j:size(del_bi,2)                  
    for l=1:size(del_bi,3)
        states_p(:,k,l) = states_p(:,k-1,l) + dt*(Afull*states_p(:,k-1,l)+Bfull*[del_bi(1,k-1,l);del_ai(1,k-1,l);del_pi(1,k-1,l);del_ci(1,k-1,l)]);
